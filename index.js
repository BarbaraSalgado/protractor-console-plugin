...
ConsolePlugin.prototype.setup = function () {

    var context = this;

    if (context.config.runAfter === 'describe' || !context.config.runAfter) {
        /**
        ...
        ConsolePlugin.prototype.teardown = function () {
            if (context.config.runOnSuites.indexOf(browser.getProcessedConfig().value_.suite) > -1)
                return ConsolePlugin.parseLog(context);
        };
    } else if (context.config.runAfter === 'it') {
        /**
         * Gather the console logs and output them as test results.  See the
